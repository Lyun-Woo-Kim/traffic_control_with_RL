"""
레이싱 게임 에이전트 - Dueling Double DQN + Dual Head 버전
- Double DQN: Q값 과대평가 방지
- Dueling DQN: Q = V(s) + A(s,a) 분해로 효율적 학습
- Dual Head: Action + Duration 동시 예측
...
class DuelingDoubleDQN_DualHead:
    """
    Dueling Double DQN + Dual Head 에이전트
    - Action과 Duration을 동시에 예측
    - Loss = action_loss + 0.2 * duration_loss
    """
    
    def __init__(self, input_size=13, action_size=8, duration_size=20,
    ...
    def train_step(self, batch_size, target_net):
        """
        Dueling Double DQN + Dual Head 학습 스텝
        ...
        with torch.no_grad():
            # Policy net으로 다음 상태에서 최적 action/duration 선택
            next_action_q_policy, next_duration_q_policy = self.model(next_states)
            best_actions = next_action_q_policy.argmax(dim=1)
            best_durations = next_duration_q_policy.argmax(dim=1)
            
            # Target net으로 선택된 action/duration의 Q값 평가
            next_action_q_target, next_duration_q_target = target_net.model(next_states)
            max_next_action_q = next_action_q_target.gather(1, best_actions.unsqueeze(1)).squeeze(1)
            max_next_duration_q = next_duration_q_target.gather(1, best_durations.unsqueeze(1)).squeeze(1)
...
def evaluate_model(agent, game, ori_checkpoints, max_time, num_tests=10):
    ...
    for test_idx in range(num_tests):
        ...
        while True:
            ...
            # greedy=True로 순수 policy만 사용 (탐험 없음)
            _, _, action, duration_idx, duration = agent.predict(state, greedy=True)
...
def train_headless(max_episode=10000, action_size=8, duration_size=20, 
    ...
    log_filename = f"DDDQN_DualHead_log_lr_{lr_str}_L{layer_num}_S{max_size}.txt"
    ...
    log_and_print("=" * 60)
    log_and_print("🚀 DUELING DOUBLE DQN + DUAL HEAD TRAINING")
    ...
    policy_net = DuelingDoubleDQN_DualHead(
        input_size=28, action_size=action_size, duration_size=duration_size,
            ...
            if result['is_goal']:
                ...
                if goal_counts > 50:
                    ...
                    if all_success:
                        ...
                        if avg_speed > best_avg_speed: 
                            lr_str = str(lr).replace(".", "_")
                            old_path = f'dddqn_dualhead_best_lr_{lr_str}_L{layer_num}_S{max_size}_E{save_episode}_T{str(round(best_avg_speed, 3)).replace(".", "_")}.pth'
                            if os.path.exists(old_path):
                                os.remove(old_path)
                            
                            best_avg_speed = avg_speed
                            save_episode = episode
                            save_path = f'dddqn_dualhead_best_lr_{lr_str}_L{layer_num}_S{max_size}_E{save_episode}_T{str(round(best_avg_speed, 3)).replace(".", "_")}.pth'
