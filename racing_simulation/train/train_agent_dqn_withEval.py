...
def get_data(game, standard_cp=None, dis_gap=None): 
    ...
    car_features = [
        ...
        game.car.brake_force/1000,                  # 브레이크 효율 
...
def train_headless(max_episode=10000, output_size=6, replay_length=100000, 
    ...
    log_filename = f"{os.path.join(current_dir, '..')}/log_files/DQN_withEval_log_lr_{lr_str}_L{layer_num}_S{max_size}.txt"
                ...
                if goal_counts > 50:
                    ...
                    if all_success:
                        ...
                        if avg_speed > best_avg_speed: 
                            # 이전 best 모델 삭제
                            lr_str = str(lr).replace(".", "_")
                            old_path = f"{os.path.join(current_dir, '..')}/example_models/dqn_best_lr_{lr_str}_L{layer_num}_S{max_size}_E{save_episode}_T{str(round(best_avg_speed, 3)).replace('.', '_')}.pth"
                            ...
                            save_path = f"{os.path.join(current_dir, '..')}/example_models/dqn_best_lr_{lr_str}_L{layer_num}_S{max_size}_E{save_episode}_T{str(round(best_avg_speed, 3)).replace('.', '_')}.pth"
