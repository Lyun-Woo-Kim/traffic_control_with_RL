...
import math
import numpy as np
from racing_game_2d import RacingGame  # headless 모드 지원 버전
import torch
...
class DQN:
    ...
    #         if not greedy and random.random() < self.epsilon:
    #             # Exploration: 랜덤 action과 duration 선택
    #             action = random.randint(0, self.action_size - 1)
    #             duration_idx = random.randint(0, self.duration_size - 1)
    #         else:
    #             # Exploitation: Q값 기반 선택
    #             action = action_q.argmax().item()
    #             duration_idx = duration_q.argmax().item()
    ...
    def get_frame_reward(self, state, coll, goal, curr_distance, curr_time, max_time, 
        ...
        return reward
    # def get_frame_reward(self, state, coll, goal, curr_distance, curr_time, max_time, 
    #                  cp_r, dis_gap, action_index, max_speed): 
    #     speed_norm = state[10]  # 0~1
    ...
    #     elif goal: 
    #         time_ratio = curr_time / (max_time * 1000)
    #         time_bonus = (1 - time_ratio) ** 2 * 50.0
    #         r = 100 + time_bonus  # 100 ~ 150
    #     else: 
    #         r = 0
        
    #     # 2. 진행 보상 (프레임당)
    #     r_dis = (dis_gap - curr_distance) * 0.05  # 기존 5.0 → 0.05
        
    #     # 3. 시간 패널티 (고정값!)
    #     r_time = 0.05  # 매 프레임 -0.05 (1초 = -3)
        
    #     # 4. 속도 보상 (대폭 축소)
    #     r_speed = speed_norm * 0.3  # 0 ~ 0.3 /frame
        
    #     # 5. 저속 패널티
    #     if speed_norm < 0.2:
    #         r_speed -= 0.2  # 너무 느리면 패널티
        
    #     # 6. 드리프트 보너스 (조건 완화)
    #     drift_bonus = 0
    #     if action_index in [3, 4]:
    #         if speed_norm > 0.15:  # 기존 0.23 → 0.15
    #             drift_bonus = speed_norm * 0.5  # 0 ~ 0.5 /frame
    #         elif speed_norm < 0.1:
    #             drift_bonus = -0.1
        
    #     # 7. 체크포인트 (외부에서 cp_r = 20 정도로)
...
def train_headless(max_episode=10000, action_size=8, duration_size=20, 
    ...
    log_filename = f"DQN_DualHead_log_lr_{lr_str}_L{layer_num}_S{max_size}.txt"
            ...
            if result['is_goal']:
                ...
                if goal_counts > 50:
                    ...
                    if all_success:
                        ...
                        if avg_speed > best_avg_speed: 
                            lr_str = str(lr).replace(".", "_")
                            old_path = f'dqn_dualhead_best_lr_{lr_str}_L{layer_num}_S{max_size}_E{save_episode}_T{str(round(best_avg_speed, 3)).replace(".", "_")}.pth'
                            if os.path.exists(old_path):
                                os.remove(old_path)
                            
                            best_avg_speed = avg_speed
                            save_episode = episode
                            save_path = f'dqn_dualhead_best_lr_{lr_str}_L{layer_num}_S{max_size}_E{save_episode}_T{str(round(best_avg_speed, 3)).replace(".", "_")}.pth'
